...
var FormUtil = {
  ...
  resolveImports: function resolveImports(rule) {
    return new Promise(function(resolve) {
      ...
      var importableRulesPromises = rule.fields
        .filter(function importableRulesFilter(fieldDef) {
      ...
      Promise.all(importableRulesPromises).then(function importableRulesPromises2(arrayOfRules) {
        ...
        if (missingImports.length > 0) {
          Notification.create(
            chrome.i18n.getMessage("notification_import_without_rule"),
            null,
            function importWithoutRule() {
        ...
        arrayOfRules
          .filter(function arrayOfRulesFilter(aRule) {
            return aRule !== null;
          })
        ...
        if (Object.keys(lookup).length > 0) {
          ...
          rule.fields.forEach(function resolveImportsFields(field, fieldIndex) {
            ...
            if (
              typeof field.import !== "undefined" &&
              typeof lookup[field.import] !== "undefined"
            ) {
  ...
  reportErrors: function(theErrors, rule, port) {
    ...
    if (theErrors.length > 0) {
      ...
      Notification.create(
        chrome.i18n.getMessage("bg_error_while_filling", [theErrors.length]),
        null,
        Utils.openOptions
      );
    }
    ...
    return new Promise(function(resolve) {
      ...
      chrome.tabs.executeScript(null, { file: pathToScript }, function() {
        ...
        chrome.tabs.executeScript(
          { code: "Libs.add('" + nameOnLib + "', window." + nameOnWindow + ");" },
          function() {
  ...
  generateLibsPromisesForContentPage: function(rule) {
    ...
    if (libs.length > 0) {
      ...
      libs.forEach(function(libPath) {
        prUsedLibs.push(
          FormUtil.injectAndAttachToLibs(
            libPath,
            Utils.vendoredLibs[libPath].name,
            Utils.vendoredLibs[libPath].onWindowName
          )
        );
      });
    }
  ...
  createGrabber: function(lastActiveTabId) {
    ...
    return function theRealGrabber(selector) {
      return new Promise(function(resolve) {
        ...
        chrome.tabs.sendMessage(lastActiveTabId, grabberMessage, function returnFromContentGrabber(
          content
        ) {
  ...
  storage: {
    ...
    set: function(key, value) {
      ...
      chrome.tabs.sendMessage(state.lastActiveTab.id, {
        action: "storageSet",
        key: key,
        value: this.base[key],
      });
  ...
  generateFunctionsPromises: function(beforeOrAfter, rule, context) {
    ...
    if (typeof rule[beforeOrAfter] === "function") {
      ...
      typeof rule[beforeOrAfter] === "object" &&
      typeof rule[beforeOrAfter].length !== "undefined"
    ) {
  ...
  handleApplyErrors: function(errors, rule) {
    ...
    errors = errors.map(function filteredErrorsMap(errorObj) {
      return {
        selector: "Inside before function",
        value: errorObj.error.beforeFunction,
  ...
  applyRule: function applyRule(rule, lastActiveTab) {
    ...
    Libs.import().then(function() {
      ...
      var beforePromises = FormUtil.generateFunctionsPromises(
        "before",
        rule,
        FormUtil.createContext()
      );
      ...
      Promise.all(beforePromises)
        .then(function beforeFunctionsPromise(beforeData) {
          ...
          var contentLibImportAndSetupContentPrs = FormUtil.generateLibsPromisesForContentPage(
            rule
          );

          // setupContent function defined? Add those to the promises
          if (typeof rule.setupContent === "function") {
            contentLibImportAndSetupContentPrs.push(
              FormUtil.generateSetupContentPromise(rule.setupContent, port)
            );
          }
  ...
  handleContentMessages: function(message) {
    ...
    if (message.action === "fillFieldFinished" && typeof FormUtil.lastRule !== "undefined") {
      ...
      if (typeof rule.teardownContent === "function") {
        FormUtil.getPort().postMessage({
          action: "teardownContent",
          value: JSONF.stringify(rule.teardownContent),
        });
      }
      ...
      if (typeof rule.after === "function") {
        Promise.all(
          FormUtil.generateFunctionsPromises("after", rule, FormUtil.createContext())
        ).then(function() {
